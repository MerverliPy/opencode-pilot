...
export async function loadLastSessionId(serverId: string): Promise<string | null> {
  return SecureStore.getItemAsync(`${KEY_LAST_SESSION}.${serverId}`);
}

export async function saveLastSessionId(serverId: string, sessionId: string): Promise<void> {
...
export function basicAuthHeader(server: ServerConfig): Record<string, string> {
  ...
  const encoded = typeof g.btoa === 'function' ? g.btoa(`${user}:${pass}`) : base64(`${user}:${pass}`);
