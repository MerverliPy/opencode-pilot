...
```bash
python3 "${OPENCODE_PLUGIN_ROOT}/skills/continuous-learning-v2/scripts/instinct-cli.py" import <file-or-url> [--dry-run] [--force] [--min-confidence 0.7]
```

Or if `OPENCODE_PLUGIN_ROOT` is not set (manual installation):

```bash
python3 ~/.opencode/skills/continuous-learning-v2/scripts/instinct-cli.py import <file-or-url>
```
...
5. Save to `~/.opencode/homunculus/instincts/inherited/`
...
New instincts saved to: ~/.opencode/homunculus/instincts/inherited/
