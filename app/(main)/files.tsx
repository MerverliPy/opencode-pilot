...
export default function FilesScreen() {
  ...
  const [path, setPath] = useState(workdir ?? ".");
  const [nodes, setNodes] = useState<FileNode[]>([]);
  const [loading, setLoading] = useState(false);

  // Sync path to workdir when it changes (e.g. user picks a new directory).
  useEffect(() => {
    if (workdir) setPath(workdir);
  }, [workdir]);
        ...
        <>
          ...
          >
            <TextInput
              ...
              placeholder={
                mode === "name" ? "fuzzy file name…" : "text in files…"
              }
          ...
          {mode === "name" ? (
            <FlatList
              ...
              ListEmptyComponent={
                <EmptyHint
                  text={
                    query.trim() ? "no matches" : "type to search file names"
                  }
                />
              }
              renderItem={({ item }) => (
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <EmptyHint
                  text={
                    query.trim() ? "no matches" : "type to search file contents"
                  }
                />
              }
              renderItem={({ item }) => (
                <TextHitRow hit={item} onPress={() => open(item.path)} />
              )}
            />
          )}
        </>
...
function FileRow({ node, onPress }: { node: FileNode; onPress: () => void }) {
  ...
  return (
    ...
    >
      <Text
        style={{
          color: isDir ? colors.accent : colors.muted,
          fontFamily: fonts.mono,
          fontSize: fontSizes.md,
          width: 24,
        }}
