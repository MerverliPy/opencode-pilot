...
- `cost`, `share.url`, `tokens.reasoning` — tracked as `[~]` partial features in ROADMAP/FEATURE-ROADMAP; types remain for API compatibility, UI is pending (not a bug)
- `audit.sh` documentation (low) — see BENCH.md section 5 for full audit suite docs
