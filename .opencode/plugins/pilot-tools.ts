...
function repoRoot(context: { directory?: string; worktree?: string }): string {
...
function verificationCommands(classifications: FileClassification[]): string[] {
  ...
  if (risks.has("terminal-stream") || risks.has("proxy-tunnel") || risks.has("auth-session") || risks.has("sqlite-memory")) {
    commands.push("npm run typecheck -w server");
  }
  if (risks.has("react-render") || risks.has("zustand-state")) {
    commands.push("npm run typecheck -w ui", "npm run test -w ui");
  }

  if (commands.length === 0 && classifications.every((entry) => entry.risks.includes("docs-only"))) {
...
function formatClassifications(classifications: FileClassification[]): string {
  ...
  return classifications
    .map((entry) => `- ${entry.file} [${entry.workspace}] risks=${entry.risks.join(",")}`)
    .join("\n");
}

export const PilotToolsPlugin: Plugin = async () => {
  return {
    tool: {
      pilot_changed_files: tool({
      ...
      pilot_risk_scan: tool({
        ...
        async execute(args, context) {
          const root = repoRoot(context);
          const files = args.files?.length ? args.files : changedFiles(root);
          const classifications = classify(files);
          const risks = unique(classifications.flatMap((entry) => entry.risks));
