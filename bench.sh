...
#
# Runs all four benchmark suites in sequence, merges results into a single
...
section "Phase 1 / 4 — Correctness Suite"
...
section "Phase 2 / 4 — Load Test  (peak ${VUS} VUs)"
...
section "Phase 3 / 4 — SSE Stress & App Flow Simulation"
...
section "Phase 4 / 4 — Memory Plugin Suite"

EXIT_MEMORY=0
node "${SCRIPT_DIR}/pilot-memory-bench.mjs" \
  --url "$URL" \
  --out "$JSON_FILE" \
  ${AUTH_FLAGS} || EXIT_MEMORY=$?

if [ $EXIT_MEMORY -eq 0 ]; then
  ok "Memory plugin suite passed"
else
  fail "Memory plugin suite had failures (results still recorded)"
fi
